...
// IDs.
GLuint vao, ibo, points_vbo, colors_vbo, uv_vbo, modelID, viewID, projID;
...
// Light variables.
glm::vec3 ambientColour = glm::vec3(1.0f, 0.1f, 0.1f);
...
void init(void)
{
	...
	//previous read model, view,projection in one line mvp and set in one line now it split up into 3 lines
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, alexTx);
	//g_cube.ColorShape(1.0f, 0.9f, 0.65f);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo);
	transformObject(glm::vec3(1.0f, 1.0f, 1.0f), X_AXIS, 0.0f, glm::vec3(5.0f, 3.0f, 2.0f));
